...
const DeviceController = require("../controllers/Device.ctrl");

router.get("/", (req, res) => {
  return res
    .status(200)
    .json({ message: "Welcome TMS Application, Now Route is ON!" });
});
...
//DEVICE ROUTE'S
router.get("/device/check", DeviceController.check);
router.post("/device/create", DeviceController.create);
router.get("/device/getAll", DeviceController.getAll);
